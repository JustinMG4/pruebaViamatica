...
public class JwtTokenValidator extends OncePerRequestFilter {
    ...
    private final UserRepository userRepository; // Añadir repositorio para acceder al AppUser real

    public JwtTokenValidator(JwtBuild jwtUtils, UserRepository userRepository) {
    ...
    protected void doFilterInternal(
        ...
        if (jwtToken != null && jwtToken.startsWith("Bearer ")) {
            ...
            String stringAuthorities = decodedJWT.getClaim("roles").asString();
            Collection<? extends GrantedAuthority> authorities = AuthorityUtils.commaSeparatedStringToAuthorityList(stringAuthorities);

            // Obtener el AppUser directamente del repositorio
            AppUser user = userRepository.findByUsernameOrEmail(username)
                    .orElse(null);

            if (user != null) {
                // Establecer el contexto de seguridad con el AppUser real
